...
shinyServer(function(input, output,session){
  ...
  observeEvent(input$doc_type,{observeEvent(input$search_mode,{observeEvent(input$cooccurrences,{observeEvent(input$prox,{observeEvent(input$jocker,{
    ...
    }else if(input$jocker==T & ((input$doc_type == 1 & input$search_mode == 3)|(input$doc_type == 2 & input$search_mode == 3))){
      output$instructions <- renderUI(HTML(str_c('<ul><li>Utiliser "a_" pour rechercher les n+1 grammes contenant a. "Napoléon_" renvoie code Napoléon, Napoléon Ier, Napoléon III, etc. </li><li>Dans le champ "StopW", indiquer le nombre de mots les plus fréquents à éliminer de la recherche</li><li>Un seul syntagme pour ce mode</li><li>Cliquer sur un point du graphique pour accéder aux documents dans la bibliothèque numérique correspondante</li></ul>')))
    ...
    }else if(input$doc_type==13|input$doc_type==14|input$doc_type==17|input$doc_type==18 | input$doc_type == 27 | input$doc_type == 28 | input$doc_type == 30 | input$doc_type == 31 | input$doc_type == 32| input$doc_type == 33| input$doc_type == 34| input$doc_type == 35){
  ...
  observeEvent(input$language,{
    ...
    if(input$language == 1 & input$bibli==1){
      ...
      updateSelectInput(session,"doc_type", "Corpus",choices = list("Presse britannique / BNA" = 8,"Presse australienne / Trove"=35, "Livres / Ngram Viewer Anglais" = 10),selected = 8)
