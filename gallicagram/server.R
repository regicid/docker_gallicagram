...
willisation <- function(input,will){
  ...
  for (i in pages) {
    ...
    if(isolate(input$doc_type==1)){
      will_url=str_c("https://shiny.ens-paris-saclay.fr/guni/api/gallicaRecords?terms=%22",URLencode(word),"%22&source=periodical&sort=relevance&year=",str_extract(fromm,"...."),mois,"&row_split=true&cursor=",i,"0")
    }
    if(isolate(input$doc_type==2)){
      will_url=str_c("https://shiny.ens-paris-saclay.fr/guni/api/gallicaRecords?terms=%22",URLencode(word),"%22&source=book&sort=relevance&year=",str_extract(fromm,"...."),mois,"&row_split=true&cursor=",i,"0")
    }
    if(isolate(input$doc_type)==56){
      will_url=str_c("https://shiny.ens-paris-saclay.fr/guni/api/gallicaRecords?terms=%22",URLencode(word),"%22&sort=relevance&year=",str_extract(fromm,"...."),mois,"&row_split=true&cursor=",i,"0")
    }
